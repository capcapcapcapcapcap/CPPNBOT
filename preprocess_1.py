...
node=pd.read_json("./dataset/Twibot-20/node.json")
edge=pd.read_json("./dataset/Twibot-20/edge.json")
label=pd.read_json("./dataset/Twibot-20/label.json")
split=pd.read_json("./dataset/Twibot-20/split.json")
...
cat_properties_tensor=default_profile_image_tensor.reshape([len(user),1])
...
for i in tqdm(range(len(edge))):
    try:
        source_id = edge.iloc[i]['source_id']
        target_id = edge.iloc[i]['target_id']
    except (KeyError, IndexError):
        continue
    else:
        dict[source_id].append(target_id)
